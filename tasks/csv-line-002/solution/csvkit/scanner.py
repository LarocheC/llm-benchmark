        ...
        while not self.at_end():
            ...
            if state == QUOTED:
                if d.is_quote(ch):
                    ...
                    # literal quote: consume both characters and stay inside
                    # the field. A lone quote closes the field; the caller
                    # handles whatever follows (a delimiter or end of line).
                    if d.is_quote(self.lookahead()):
                        chars.append(self.dialect.quotechar)
                        self.pos += 2
                        continue
                    self.pos += 1
                    return "".join(chars)
