...
def toposort(graph):
    ...
    # A cycle leaves some nodes unemitted. If the number of nodes we managed to
    # process differs from the number we ever made ready, a cycle is present.
    if len(state.order) != state.enqueued:
